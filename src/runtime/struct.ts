...
export default abstract class StructBase implements Struct {
    // public readonly fqtn: string;
    // constructor(fqtn: string) {
    //     this.fqtn = fqtn;
    // }
    public abstract get fqtn(): string;

    async dehydrate(content: Content) {
        content.fqtn = this.fqtn;
    }
    hydrate(context: HydrateContext) {

    }

    async toContent(): Promise<Content> {
        const content = new Content();
        await this.dehydrate(content);
        return content;
    }
}

export abstract class Problem extends Error implements Struct {
